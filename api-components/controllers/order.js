        ...
        if (response.success) {
            var request_data_body = request_data.body;
                var store  = response.store;
                        Order.findOne({_id: request_data_body.order_id, $or: [{order_status_id: ORDER_STATUS_ID.RUNNING},{order_status_id: ORDER_STATUS_ID.IDEAL}] }).then((order) => {
                            if (order) {
                                            ...
                                            Store.findOne({_id: order.store_id}).then((store) => {
                                                ...
                                                order.save().then(() => {
                                                    ...
                                                    Order_payment.findOne({_id: order.order_payment_id}).then((order_payment) => {
                                                        if (order_payment)
                                                        {
                                                            if (order_status != ORDER_STATE.WAITING_FOR_ACCEPT_STORE) {
                                                                // utils.refund_payment_for_selected_payment_gateway(order_payment, function (response) {
                                                                //     console.log(response);
                                                                // });
                                                            }
